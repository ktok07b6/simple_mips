...
const unsigned int imem[IMEM_SIZE] = {
...
//const int test_data[8] = {22, 5, -9, 3, -17, 38, 0, 11};
const int test_data[8] = {100, 90, 60, 30, 10, -20, -40, -100};

#define SIMPLE_MIPS_INPUT_ADDR ((XPAR_PS7_DDR_0_S_AXI_HIGHADDR + 1) - 0x00100000)
#define SIMPLE_MIPS_OUTPUT_ADDR (SIMPLE_MIPS_INPUT_ADDR+0x1000)
#define MODE_READ (1)
#define MODE_WRITE (0)

#define STATUS_INPUT_BURST_DONE_BIT  (1<<0)
#define STATUS_OUTPUT_BURST_DONE_BIT (1<<1)
#define STATUS_PROCESS_DONE_BIT      (1<<2)
...
int main()
{
    ...
    //clear output space
    memset((void*)SIMPLE_MIPS_OUTPUT_ADDR, 0, (IMEM_SIZE+DMEM_SIZE+1)*4);

    //set-up input data
    char *p = (char*)SIMPLE_MIPS_INPUT_ADDR;
    memcpy((void*)p, imem, 44*4);
    p += 44*4;
    memcpy((void*)p, test_data, 8*4);
    p += 8*4;
    memset((void*)p, 0, (64-8)*4);
    Xil_DCacheFlushRange(SIMPLE_MIPS_INPUT_ADDR, 1024);

    //input & invoke
    SIMPLE_MIPS_mWriteReg(XPAR_SIMPLE_MIPS_0_S_BASEADDR, SIMPLE_MIPS_S_SLV_REG0_OFFSET, MODE_READ);
    SIMPLE_MIPS_mWriteReg(XPAR_SIMPLE_MIPS_0_S_BASEADDR, SIMPLE_MIPS_S_SLV_REG1_OFFSET, SIMPLE_MIPS_INPUT_ADDR);
    SIMPLE_MIPS_mWriteReg(XPAR_SIMPLE_MIPS_0_S_BASEADDR, SIMPLE_MIPS_S_SLV_REG2_OFFSET, IMEM_SIZE+DMEM_SIZE);
    SIMPLE_MIPS_mWriteReg(XPAR_SIMPLE_MIPS_0_S_BASEADDR, SIMPLE_MIPS_S_SLV_REG3_OFFSET, 1);
    SIMPLE_MIPS_mWriteReg(XPAR_SIMPLE_MIPS_0_S_BASEADDR, SIMPLE_MIPS_S_SLV_REG3_OFFSET, 0);
    do {
    	status = SIMPLE_MIPS_mReadReg(XPAR_SIMPLE_MIPS_0_S_BASEADDR, SIMPLE_MIPS_S_SLV_REG4_OFFSET);
    } while ((status & STATUS_PROCESS_DONE_BIT) == 0);

    //output
    SIMPLE_MIPS_mWriteReg(XPAR_SIMPLE_MIPS_0_S_BASEADDR, SIMPLE_MIPS_S_SLV_REG0_OFFSET, MODE_WRITE);
    SIMPLE_MIPS_mWriteReg(XPAR_SIMPLE_MIPS_0_S_BASEADDR, SIMPLE_MIPS_S_SLV_REG1_OFFSET, SIMPLE_MIPS_OUTPUT_ADDR);
    SIMPLE_MIPS_mWriteReg(XPAR_SIMPLE_MIPS_0_S_BASEADDR, SIMPLE_MIPS_S_SLV_REG2_OFFSET, IMEM_SIZE+DMEM_SIZE+1);
    SIMPLE_MIPS_mWriteReg(XPAR_SIMPLE_MIPS_0_S_BASEADDR, SIMPLE_MIPS_S_SLV_REG3_OFFSET, 1);
    SIMPLE_MIPS_mWriteReg(XPAR_SIMPLE_MIPS_0_S_BASEADDR, SIMPLE_MIPS_S_SLV_REG3_OFFSET, 0);
    do {
    	status = SIMPLE_MIPS_mReadReg(XPAR_SIMPLE_MIPS_0_S_BASEADDR, SIMPLE_MIPS_S_SLV_REG4_OFFSET);
    } while ((status & STATUS_OUTPUT_BURST_DONE_BIT) == 0);

    Xil_DCacheInvalidateRange(SIMPLE_MIPS_OUTPUT_ADDR, 1024);
    memcpy(output, SIMPLE_MIPS_OUTPUT_ADDR + (44*4), 8*4);

    //check output data
    for (i = 0; i < 8-1; i++) {
    	Xil_AssertVoid(output[i] < output[i+1]);
    }
